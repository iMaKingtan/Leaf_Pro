...
import com.generate.common.utils.JsonUtils;
...
public class SnowflakeEtcdHolder extends AbstractGenerateLifeCycle {
    ...
    protected void doInit() {
        ...
        try {
            ...
            // 获得所有永久地址的节点信息
            GetResponse response = kvClient.get(rootNode, GetOption.newBuilder().isPrefix(true).withKeysOnly(true).build()).get();
    ...
    private void checkServersClockBack(KV kvClient) throws Exception {
        ...
        List<String> urls = keyValues.stream().filter(kv -> {
            ...
            EndPointData endPointData = JsonUtils.jsonToObject(value,EndPointData.class);
        ...
        if (absSubTime > snowflakeProperties.getBlockBackThreshold()) {
            ...
            throw new BizException("服务器节点之间存在时钟不同步问题，请检查重试,节点信息:"+ JsonUtils.objectToJson(urls));
        }
    }
    ...
    private EndPointData parseEndPointReportData(String json) {
        return JsonUtils.jsonToObject(json,EndPointData.class);
    }
